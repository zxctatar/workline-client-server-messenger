...
void ChatInfoBarController::slotSetChatData()
{
    ...
    if(isGroupChat_)
    {
        QString groupName_ = SelectedChatManager::instance().getGroupName();
        int usersCount_ = SelectedChatManager::instance().getUsersCount();

        emit setGroupChatDataSignal(groupName_, usersCount_, isGroupChat_);
    }
    else
    {
